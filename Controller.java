...
public class Controller {
    ...
    public void onClick(ActionEvent event) {
        ...
        boolean correctGuess = false; // Flag to track if the guess is correct

        for (int i = 0; i < myLetters.size(); i++) {
            if (myLetters.get(i).equals(letter)) {
                correct++;
                answer.set(i * 2, letter); // Set the guessed letter at correct position
                correctGuess = true;
            }
        }
        ...
        if (correctGuess) {
            ...
            mistakes++;
            ((Button) event.getSource()).setDisable(true);
            // Display hangman elements based on the number of mistakes
